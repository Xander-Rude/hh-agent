...
class VacancyEvaluator:
    ...
    def evaluate(
        ...
        ai_project_cover_rule = (
            "Для содержательно AI-релевантной вакансии ОБЯЗАТЕЛЬНО органично "
            "упомяни собственный AI-agent проект кандидата как практический "
            "релевантный опыт. Не подменяй этим основной опыт и не добавляй "
            "проект в обычные вакансии."
            if ai_project_cover_enabled
            else "AI-project контекст отключён аварийным флагом; не упоминай "
            "hh-agent, GitHub, rudenko.one или другие личные сайты."
        )
...
- Сначала выбери 5-7 наиболее важных требований вакансии как внутреннюю
  опору сопоставления и проверь их только по подтверждённым фактам резюме.
...
- Выбери только 1-2 наиболее релевантных факта или результата для самого
  письма, чтобы не превращать его в пересказ резюме. Каждый из них должен быть
  подтверждён резюме.
