...
EXECUTIVE_ALLOWED_PATTERNS = [
    ...
    # Russian nominative forms: директор по ИТ, руководитель департамента ИТ,
    # начальник управления информационных систем, директор по разработке, etc.
    re.compile(
        r"\b(?:руководитель|директор|начальник|глава)\s+"
        r"(?:(?:департамента|управления|службы|отдела|направления|центра)\s+)?"
        r"(?:по\s+)?(?:развити(?:ю|я)\s+)?"
    ...
    # Russian deputy/genitive forms: заместитель директора по ИТ,
    # первый заместитель генерального директора по цифровой трансформации, etc.
    re.compile(
        r"\b(?:первый\s+)?заместитель\s+"
        r"(?:(?:генерального|исполнительного)\s+)?"
        r"(?:руководителя|директора|начальника|главы)\s+"
        r"(?:(?:департамента|управления|службы|отдела|направления|центра)\s+)?"
        r"(?:по\s+)?(?:развити(?:ю|я)\s+)?"
        r"(?:it|ит|информационн\w*\s+технолог\w*|информационн\w*\s+систем\w*|"
        r"технолог\w*|цифров\w+(?:\s+(?:трансформац\w*|развити\w*|технолог\w*))?|"
    ...
    re.compile(
        r"\b(?:первый\s+)?заместитель\s+технического\s+директора\b",
        re.IGNORECASE,
    ),
    re.compile(
