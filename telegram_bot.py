...
class TelegramSingleInstanceLock:
    ...
    Keeps one byte locked for the whole lifetime of the process.
    A second bot process exits cleanly instead of starting another
    getUpdates loop and causing Telegram Conflict errors.
    """
    ...
    if evaluation.selected_resume_id:
        parts.extend(
            [
                ...
                (
                    "🎯 Match резюме: "
                    f"{evaluation.selected_resume_score or 0}%"
                ),
            ]
        )
...
def resolve_target_chat_id(
    ...
    """
    Prefer the chat that invoked the command.
    Fallback to TELEGRAM_CHAT_ID from .env only when available.
    """
    if (
        context._chat_id is not None
    ):
        return int(
            context._chat_id
        )
...
async def send_new_vacancies(
    ...
    if target_chat_id is None:
        raise RuntimeError(
            "Не удалось определить Telegram chat_id "
            "для отправки вакансий."
        )
...
def _fmt_state(
    ...
    if state.get("started_at"):
        lines.append(
            f"  старт: {state['started_at']}"
        )

    if state.get("finished_at"):
        lines.append(
            f"  финиш: {state['finished_at']}"
        )

    if state.get("updated_at"):
        lines.append(
            f"  heartbeat: {state['updated_at']}"
        )
...
def _start_pipeline_from_telegram(
    chat_id: int,
) -> int:
    python_exe = (
        ROOT
...
async def health_command(
    ...
    pipeline_state = read_state(
        PIPELINE_STATE
    )
    apply_state = read_state(
        APPLY_STATE
    )
    resume_raise_state = read_state(
        RESUME_RAISE_STATE
    )
    ...
    lines = [
        ...
        (
            "  PID: "
            f"{telegram_state.get('pid') or os.getpid()}"
        ),
        (
            "  uptime: "
            f"{_format_uptime(telegram_state.get('started_at'))}"
        ),
        "",
        *checks,
        "",
...
async def run_command(
    ...
    try:
        if update.effective_chat is None:
            raise RuntimeError(
                "Не удалось определить текущий Telegram chat_id."
            )

        pid = _start_pipeline_from_telegram(
        ...
        await update.message.reply_text(
            "▶️ Pipeline запущен.\n"
            f"PID: {pid}\n\n"
            "Я пришлю сообщения о переходе "
            "между этапами и о результате.\n"
...
def main() -> None:
    lock = TelegramSingleInstanceLock(
        TELEGRAM_LOCK_FILE
    )
    ...
    try:
            ...
            print(
                "HH Telegram Bot запущен."
            )

            print(
                "Access mode: public "
                "(commands accepted from any Telegram chat)."
            )

            print(
                f"Минимальный score: "
                f"{MIN_SCORE_TO_NOTIFY}"
            )
            ...
            print(
                "Telegram runtime state initialized.",
                flush=True,
            )
